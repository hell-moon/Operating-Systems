...
int main(int argc, char *argv[])
{
	if (argc < 4)
	{
		fprintf(stderr, "USAGE: %s encodedtext key port\n", argv[0]);
		exit(0);
	} // Check usage & args
	...
	readFileToBuffer(argv[1], &encodedLength, &encodedtext);
	readFileToBuffer(argv[2], &keyLength, &keytext);
	// printf("%d %d", encodedLength, keyLength);

	// TODO Verify key is at least as large as encodedtext
	if (keyLength < encodedLength)
	{
	...
	for (idx = 0; idx < (keyLength - 1); idx++)
	{
		...
		// TODO Send encodedtext and key to server
		sendMessageToSocket(encodedLength, encodedtext, socketFD);
